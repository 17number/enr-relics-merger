...
async function generateMergedImage() {
  ...
  if (!isiOS() && !isSafari()){
    ...
    document.getElementById('shareToXNote').style.display = 'none';
  }
...
shareToXBtn.addEventListener('click', async () => {
  try {
    ...
    console.error(err);
    alert('画像のコピーに失敗しました（ブラウザ未対応の可能性があります）');
  }
  ...
  const text = encodeURIComponent([
    lang === 'ja' ? `${title}で作成。` : `Generated by ${title}.`,
    `https://17number.github.io/enr-relics-merger/` + (lang === 'en' ? `en` : ``),
    `#NIGHTREIGN #ELDENRINGNIGHTREIGN #ナイトレイン #エルデンリングナイトレイン #遺物画像結合ツール #RelicImageCombiner`,
  ].join(`\n`));
